...
现有生产环境首次打开论坛推送页时，会从 Flarum 的实际数据库环境读取 `leo-t-notify-push.*` 和论坛 SMTP 设置并一次性导入主站 MongoDB。论坛相关更新成功后旧 `leo-t/flarum-notify-push` 扩展会被停用以避免重复发送，但插件包和原设置不会删除，回滚时仍可恢复。桥接安装器还会通过 Flarum 官方设置仓库保存事件上报地址和签名密钥，避免 PHP-FPM Worker 无法读取 Docker 环境变量时真实发帖事件被静默跳过。机器人消息使用带标题、信息分区、双时区和详情入口的卡片式 Markdown；新用户通知沿用旧插件行为，将注册邮箱发送到这些管理员渠道。此功能没有轮询，配置缓存 60 秒；事件队列最多暂存 100 个、同时处理 2 个，Redis 去重键有效 10 分钟且内存降级最多 1000 条。
