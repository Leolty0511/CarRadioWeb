...
- 后台“工具 → 二维码制作”可创建隐藏的英文多链接扫码页，支持 PNG/SVG 下载、链接排序、启停和永久删除；
...
论坛通知已经内置到现有桥接扩展。Flarum 发生新用户注册、新主题或新回复时，以 `FORUM_SSO_BRIDGE_SECRET` 对短时事件签名后提交到主站 `/api/forum-events`，再由后台“消息推送”中已启用的渠道发送。论坛事件开关（新用户、新主题、新回复，以及忽略管理员/版主发帖）与会员注册、知识库留言一起配置在该页，测试按钮也使用同一套渠道。`FORUM_EVENT_URL` 由论坛部署/桥接脚本写入 `.env.flarum`；主站限制通知链接只能指向当前论坛。

论坛相关更新成功后旧 `leo-t/flarum-notify-push` 扩展会被停用以避免重复发送，但插件包和原设置不会删除，回滚时仍可恢复。桥接安装器还会通过 Flarum 官方设置仓库保存事件上报地址和签名密钥，避免 PHP-FPM Worker 无法读取 Docker 环境变量时真实发帖事件被静默跳过。机器人消息使用带标题、信息分区和详情入口的卡片式 Markdown；新用户通知会把注册邮箱发送到管理员渠道。此功能没有轮询；事件队列最多暂存 100 个、同时处理 2 个，Redis 去重键有效 10 分钟且内存降级最多 1000 条。
