...
export interface DashboardData { blockedIps: number; suspiciousIps: number; attacksToday: number; activeIps: number; todayBans: number; recentEvents: SecurityEvent[]; topRequestIps: SecurityIp[]; topAttackIps: Array<{ _id: string; count: number; lastRule: string }> }
