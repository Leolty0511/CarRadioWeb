...
import { Check, ChevronLeft, ChevronRight, Copy, Search, UserCheck, X } from 'lucide-react'
...
export function MemberManagement() {
  ...
  const load = async (requestedPage = page) => {
    if (members.length === 0 && total === 0) {setLoading(true)}
  ...
  const review = async (member: MemberRecord, status: 'active' | 'rejected' | 'suspended') => {
    ...
    if (result.success) { showToast({ type: 'success', title: '会员状态已更新' }); await load(page) }
    else {showToast({ type: 'error', title: '更新失败' })}
  }

  const createInvite = async () => {
    const result = await createMemberInvitation({ maxUses: Number(maxUses) || 1, note })
    if (result.success) { setNewCode(result.code); setNote(''); await load(page) }
