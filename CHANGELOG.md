...
- 修复旧版后台更新器在 PM2 环境未传入容器变量时无法完成更新前备份的问题：现在会自动识别 `automotivehu-mongo`、`flarum_db` 和 `flarum_app`，并可根据当前项目的 Docker Compose 标签发现改名后的容器；数据库名优先读取当前 MongoDB URI，账号和密码复用容器现有配置，仍保持备份失败即停止更新。资源包下载改为直接流式写盘，并新增低内存资源包安装脚本，可在后台更新被旧版备份逻辑阻断时通过远程连接安全升级，不会重置论坛插件或设置。
- 修复生产环境未安装数据库命令行工具时，更新前自动备份直接阻止后台更新的问题：MongoDB 增加保留 BSON 类型和集合索引的低内存流式 EJSON 备份与恢复工具；Flarum 自动识别 `flarum_db`、复用 `.env.flarum` 中的现有数据库密码，并使用一致性逐行导出；继续保持备份失败即停止更新的保护策略。
