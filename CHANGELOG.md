...
- 将部署依赖锁定到官方 npm 源，避免镜像源异常导致发布包生成失败。

## [1.1.10] - 2026-08-07

### Fixes

- 修复无 `.git` 的预构建生产部署无法拉取更新的问题。
- 更新器会校验部署包提交号，并在预构建部署中跳过不必要的 Git 操作。

## [1.1.9] - 2026-08-07

### Fixes

- 修复后台从 GitHub 拉取更新时只重启后端的问题。
- 更新完成后自动刷新独立前端进程或 Nginx，并在随后重启后端和执行健康检查。
- 修复 CI 前端任务缺少后端依赖导致的类型检查和构建失败。
