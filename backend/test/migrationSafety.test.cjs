...
test('forum event transport settings are available to PHP-FPM through Flarum settings', () => {
  ...
  assert.match(installer, /carradioweb-forum-bridge\.site_url/)
  const middleware = fs.readFileSync(path.join(__dirname, '..', '..', 'forum-extensions', 'carradioweb-forum-bridge', 'src', 'ForumBridgeMiddleware.php'), 'utf8')
  assert.match(middleware, /\/api\/site-settings\?language=en/)
  assert.match(middleware, /logoText/)
  assert.match(middleware, /siteName/)
  assert.match(middleware, /Back to Website/)
  assert.match(middleware, /injectHomeSiteLink/)
