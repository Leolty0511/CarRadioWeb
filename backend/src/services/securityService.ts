...
export function getTrustedClientIp(req: Request): string {
  ...
  const trusted = new Set((process.env.TRUSTED_PROXY_IPS || '127.0.0.1,::1').split(',').map(v => v.trim()).filter(Boolean))
  const fromProxy = trusted.has(remote) || trusted.has(req.ip?.replace(/^::ffff:/, '') || '')
  if (fromProxy) {
...
async function getSettings() {
  ...
  const value = (await SecuritySettings.findOne({ key: 'default' }).lean()) || DEFAULT_SETTINGS
...
async function getWhitelist() {
  ...
  const items = await SecurityWhitelist.find().select('ip').lean()
...
export function invalidateSecurityCaches() { settingsCache = null; whitelistCache = null }
...
async function redisCount(ip: string): Promise<number> {
  ...
  const found = queue.filter(item => item.ip === ip && Date.now() - Number(item.time || 0) < 60_000).length
  return found
}
...
export function securityTrackingMiddleware(req: Request, res: Response, next: NextFunction): void { const started = Date.now(); res.on('finish', () => { void trackRequest(req, res, Date.now() - started) }); next() }
