...
import { startProjectUpdateCheckScheduler } from './services/projectUpdateService';
...
// IP 安全中心：先拦截已封禁来源，再用 Redis 高频计数并批量聚合落库。
...
const server = app.listen(PORT, () => {
  ...
  startProjectUpdateCheckScheduler();
  systemLogger.info('Project update checks scheduled every 3 days');
});
